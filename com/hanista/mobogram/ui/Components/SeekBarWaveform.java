...
public class SeekBarWaveform {
    ...
    public void draw(Canvas canvas) {
        if (this.waveformBytes != null && this.width != 0) {
            ...
            if (dp > 0.1f) {
                ...
                int i = (this.messageObject == null || this.messageObject.isOutOwner() || !this.messageObject.isContentUnread() || this.messageObject.messageOwner.to_id.channel_id != 0) ? this.selected ? this.selectedColor : this.innerColor : this.outerColor;
                paint.setColor(i);
                    ...
                    if (i3 != i) {
                        ...
                        for (int i12 = 0; i12 < i6; i12++) {
                            ...
                            if (dp3 >= this.thumbX || AndroidUtilities.dp(2.0f) + dp3 >= this.thumbX) {
                                canvas.drawRect((float) dp3, (float) (AndroidUtilities.dp(14.0f - Math.max(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, (14.0f * ((float) i5)) / 31.0f)) + dp2), (float) (AndroidUtilities.dp(2.0f) + dp3), (float) (AndroidUtilities.dp(14.0f) + dp2), paintInner);
                                if (dp3 < this.thumbX) {
                                    canvas.drawRect((float) dp3, (float) (AndroidUtilities.dp(14.0f - Math.max(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, (14.0f * ((float) i5)) / 31.0f)) + dp2), (float) this.thumbX, (float) (AndroidUtilities.dp(14.0f) + dp2), paintOuter);
                                }
                            } else {
                                canvas.drawRect((float) dp3, (float) (AndroidUtilities.dp(14.0f - Math.max(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, (14.0f * ((float) i5)) / 31.0f)) + dp2), (float) (AndroidUtilities.dp(2.0f) + dp3), (float) (AndroidUtilities.dp(14.0f) + dp2), paintOuter);
                            }
                            i11++;
                        }
                        i = i11;
    ...
    public boolean onTouch(int i, float f, float f2) {
        if (i == 0) {
            ...
            if (this.startX == Face.UNCOMPUTED_PROBABILITY || Math.abs(f - this.startX) <= AndroidUtilities.getPixelsInCM(DefaultLoadControl.DEFAULT_LOW_BUFFER_LOAD, true)) {
                return true;
            }
            ...
            this.startX = Face.UNCOMPUTED_PROBABILITY;
            return true;
        }
        return false;
    }
