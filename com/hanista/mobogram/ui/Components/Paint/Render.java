...
public class Render {
    private static RectF Draw(RenderState renderState) {
        ...
        while (i2 < count) {
            ...
            asFloatBuffer.put(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            asFloatBuffer.put(0.0f);
            asFloatBuffer.put(read5);
            i3++;
    ...
    private static void PaintSegment(Point point, Point point2, RenderState renderState) {
        ...
        float atan2 = Math.abs(renderState.angle) > 0.0f ? renderState.angle : (float) Math.atan2(substract.f2680y, substract.f2679x);
        float f = renderState.baseWeight * renderState.scale;
        double max = (double) Math.max(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, renderState.spacing * f);
        Point multiplyByScalar = distanceTo > 0.0d ? substract.multiplyByScalar(1.0d / distanceTo) : point3;
        float min = Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, renderState.alpha * 1.15f);
