...
import com.hanista.mobogram.messenger.audioinfo.mp3.MP3Frame.Header;
...
public class MP3Info extends AudioInfo {
    ...
    /* renamed from: com.hanista.mobogram.messenger.audioinfo.mp3.MP3Info.1 */
    class C06871 implements StopReadCondition {
        final long stopPosition;
        final /* synthetic */ long val$fileLength;

        C06871(long j) {
            this.val$fileLength = j;
            this.stopPosition = this.val$fileLength - 128;
        }
    ...
    static {
        LOGGER = Logger.getLogger(MP3Info.class.getName());
    }
    ...
    public MP3Info(InputStream inputStream, long j, Level level) {
        ...
        if (this.duration <= 0 || this.duration >= 3600000) {
            try {
                this.duration = calculateDuration(mP3Input, j, new C06871(j));
            } catch (Throwable e) {
    ...
    long calculateDuration(MP3Input mP3Input, long j, StopReadCondition stopReadCondition) {
        ...
        if (readFirstFrame != null) {
            ...
            Object obj = null;
            int duration = AdaptiveEvaluator.DEFAULT_MIN_DURATION_FOR_QUALITY_INCREASE_MS / readFirstFrame.getHeader().getDuration();
            while (true) {
    ...
    MP3Frame readFirstFrame(MP3Input mP3Input, StopReadCondition stopReadCondition) {
        ...
        while (read != -1) {
            if (i == 255 && (read & PsExtractor.VIDEO_STREAM) == PsExtractor.VIDEO_STREAM) {
                mP3Input.mark(2);
                ...
                if (header != null) {
                    ...
                    try {
                        ...
                        if (!mP3Frame.isChecksumError()) {
                            ...
                            if (i == NalUnitUtil.EXTENDED_SAR && (read2 & 254) == (read & 254)) {
                                i = stopReadCondition.stopRead(mP3Input) ? -1 : mP3Input.read();
    ...
    MP3Frame readNextFrame(MP3Input mP3Input, StopReadCondition stopReadCondition, MP3Frame mP3Frame) {
        ...
        if (read == NalUnitUtil.EXTENDED_SAR && (read2 & PsExtractor.VIDEO_STREAM) == PsExtractor.VIDEO_STREAM) {
            int read3 = stopReadCondition.stopRead(mP3Input) ? -1 : mP3Input.read();
