...
class zzb<TResult, TContinuationResult> implements OnFailureListener, OnSuccessListener<TContinuationResult>, zzf<TResult> {
    ...
    class C02511 implements Runnable {
        ...
        C02511(zzb com_google_android_gms_tasks_zzb, Task task) {
            this.aJy = com_google_android_gms_tasks_zzb;
            this.aJw = task;
        }

        public void run() {
            try {
                Task task = (Task) this.aJy.aJu.then(this.aJw);
                if (task == null) {
    ...
    public void onComplete(@NonNull Task<TResult> task) {
        this.aBG.execute(new C02511(this, task));
    }
