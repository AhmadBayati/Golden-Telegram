...
public class ViewDragHelper {
    ...
    static class C00781 implements Interpolator {
        C00781() {
        }
    ...
    static {
        sInterpolator = new C00781();
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C00792();
        if (viewGroup == null) {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        i4 = Math.abs(i2);
        return Math.min(i4 > 0 ? Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) i4)) * 1000.0f) * EDGE_TOP : (int) (((((float) Math.abs(i)) / ((float) i3)) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) * 256.0f), MAX_SETTLE_DURATION);
    }
    ...
    public void processTouchEvent(MotionEvent motionEvent) {
        ...
        switch (actionMasked) {
            case STATE_IDLE /*0*/:
                x = motionEvent.getX();
                y = motionEvent.getY();
                i = motionEvent.getPointerId(STATE_IDLE);
