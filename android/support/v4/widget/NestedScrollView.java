...
public class NestedScrollView extends FrameLayout implements NestedScrollingChild, NestedScrollingParent, ScrollingView {
    ...
    protected float getBottomFadingEdgeStrength() {
        ...
        return bottom < verticalFadingEdgeLength ? ((float) bottom) / ((float) verticalFadingEdgeLength) : DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    }
    ...
    protected float getTopFadingEdgeStrength() {
        ...
        int verticalFadingEdgeLength = getVerticalFadingEdgeLength();
        int scrollY = getScrollY();
        return scrollY < verticalFadingEdgeLength ? ((float) scrollY) / ((float) verticalFadingEdgeLength) : DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (this.mFillViewport && MeasureSpec.getMode(i2) != 0 && getChildCount() > 0) {
            ...
            if (childAt.getMeasuredHeight() < measuredHeight) {
                childAt.measure(getChildMeasureSpec(i, getPaddingLeft() + getPaddingRight(), ((FrameLayout.LayoutParams) childAt.getLayoutParams()).width), MeasureSpec.makeMeasureSpec((measuredHeight - getPaddingTop()) - getPaddingBottom(), C0700C.ENCODING_PCM_32BIT));
            }
        }
    }
                        ...
                        if (!dispatchNestedScroll(0, scrollY2, 0, i - scrollY2, this.mScrollOffset)) {
                            if (obj != null) {
                                ...
                                if (actionMasked < 0) {
                                    ...
                                    this.mEdgeGlowBottom.onPull(((float) i) / ((float) getHeight()), DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - (motionEvent.getX(findPointerIndex) / ((float) getWidth())));
                                    if (!this.mEdgeGlowTop.isFinished()) {
                                        this.mEdgeGlowTop.onRelease();
                                    }
                                }
