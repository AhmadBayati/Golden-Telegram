...
public class PagerTitleStrip extends ViewGroup {
    ...
    public PagerTitleStrip(Context context, AttributeSet attributeSet) {
        ...
        this.mLastKnownPositionOffset = Face.UNCOMPUTED_PROBABILITY;
    ...
    protected void onMeasure(int i, int i2) {
        if (MeasureSpec.getMode(i) != C0700C.ENCODING_PCM_32BIT) {
            throw new IllegalStateException("Must measure with an exact width");
        }
        int paddingTop = getPaddingTop() + getPaddingBottom();
        int childMeasureSpec = getChildMeasureSpec(i2, paddingTop, -2);
        int size = MeasureSpec.getSize(i);
        int childMeasureSpec2 = getChildMeasureSpec(i, (int) (((float) size) * DefaultLoadControl.DEFAULT_LOW_BUFFER_LOAD), -2);
    ...
    public void setNonPrimaryAlpha(@FloatRange(from = 0.0d, to = 1.0d) float f) {
        this.mNonPrimaryAlpha = ((int) (255.0f * f)) & NalUnitUtil.EXTENDED_SAR;
    ...
    void updateText(int i, PagerAdapter pagerAdapter) {
        ...
        count = MeasureSpec.makeMeasureSpec(Math.max(0, (int) (((float) ((getWidth() - getPaddingLeft()) - getPaddingRight())) * DefaultLoadControl.DEFAULT_HIGH_BUFFER_LOAD)), TLRPC.MESSAGE_FLAG_MEGAGROUP);
        int makeMeasureSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (getHeight() - getPaddingTop()) - getPaddingBottom()), TLRPC.MESSAGE_FLAG_MEGAGROUP);
