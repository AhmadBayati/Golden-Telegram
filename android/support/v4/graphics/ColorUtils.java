...
public final class ColorUtils {
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        float abs2 = abs * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs(((f / BitmapDescriptorFactory.HUE_YELLOW) % 2.0f) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        switch (((int) f) / 60) {
        ...
        return Color.rgb(constrain(round, 0, (int) NalUnitUtil.EXTENDED_SAR), constrain(round2, 0, (int) NalUnitUtil.EXTENDED_SAR), constrain(round3, 0, (int) NalUnitUtil.EXTENDED_SAR));
    }
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull float[] fArr) {
        ...
        fArr[MIN_ALPHA_SEARCH_PRECISION] = constrain(f2, 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        return Color.rgb(constrain((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : d4 * 12.92d) * 255.0d), 0, (int) NalUnitUtil.EXTENDED_SAR), constrain((int) Math.round((d5 > 0.0031308d ? (1.055d * Math.pow(d5, 0.4166666666666667d)) - 0.055d : 12.92d * d5) * 255.0d), 0, (int) NalUnitUtil.EXTENDED_SAR), constrain((int) Math.round((d6 > 0.0031308d ? (1.055d * Math.pow(d6, 0.4166666666666667d)) - 0.055d : 12.92d * d6) * 255.0d), 0, (int) NalUnitUtil.EXTENDED_SAR));
    }
    ...
    public static int calculateMinimumAlpha(@ColorInt int i, @ColorInt int i2, float f) {
        ...
        if (Color.alpha(i2) != NalUnitUtil.EXTENDED_SAR) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(i2));
        } else if (calculateContrast(setAlphaComponent(i, NalUnitUtil.EXTENDED_SAR), i2) < ((double) f)) {
            return -1;
        } else {
            int i5 = 0;
